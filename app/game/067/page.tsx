...
export default function Page() {
  ...
  const connectToRoom = useCallback((rid: string, name: string, creator: boolean) => {
    ...
    ch.on('broadcast', { event: 'gameState' }, (payload) => {
      ...
      if (s.type === 'join') {
        ...
        if (me2) { setMyDice(me2.dice || []); setMyHand(me2.dice ? calcHand(me2.dice, oneSealed) : null); }
        ...
        setPhase(s.phase);
        if (s.bidHistory) setBidHistory(s.bidHistory);
        if (s.oneSealed !== undefined) setOneSealed(s.oneSealed);
        if (s.players) {
          setPlayers(s.players);
          const me3 = s.players.find((p: any) => p.name === playerName);
          if (me3) { setMyDice(me3.dice || []); setMyHand(me3.dice ? calcHand(me3.dice, oneSealed) : null); }
        }
      }
     else if (s.type === 'open') {
        setGameOver(true);
  ...
  const handleBeginGame = useCallback(async () => {
    ...
    broadcastState({ type: "start", players: newPlayers, currentPlayer: firstPlayer, phase: "bidding", cupOpened: false, oneSealed: false });
  ...
  const handleBid = useCallback(async (count: number, value: number) => {
    ...
    if (oneSealed && value === 1) { setErrorMsg('1号已被封印，只能叫2-6'); return; }
  ...
  }, [currentPlayer, playerName, gameOver, lastBid, players, broadcastState, addLog, oneSealed, bidHistory]);
          ...
          {players.map((p: any, i: number) => {
            ...
            return (
              ...
              >
                ...
                {showingDice && isMe && hasDice && (<div style={{ fontSize: 9, color: "#fbbf24", marginTop: 2 }}>👁 查看中 (3秒后自动盖)</div>)}
                {!showingDice && hasDice && isMe && gameStarted && !gameOver && (
          ...
          {gameStarted && !gameOver && phase === 'bidding' && isMyTurn && (
            <>
              ...
              <div style={{ marginBottom: 8, display: 'flex', gap: 4, justifyContent: 'center', flexWrap: 'wrap' }}>
                ...
                {(!oneSealed ? [1,2,3,4,5,6] : [2,3,4,5,6]).map(v => (
                  <button
                    key={v}
                    style={{
                      ...S.btnBid,
